...
public class UserService {
    ...
    public boolean existsByEmailDirect(String email) {
        return userMapper.selectOne(new LambdaQueryWrapper<User>().eq(User::getEmail, email)) != null;
    }
    ...
    public void cacheUser(User user) {
        ...
        redisUtil.set(USER_CACHE_PREFIX + user.getId(), user, USER_CACHE_EXPIRE);
        if (user.getUsername() != null) {
            redisUtil.set(USER_CACHE_PREFIX + "username:" + user.getUsername(), user, USER_CACHE_EXPIRE);
        }
        if (user.getEmail() != null) {
            redisUtil.set(USER_CACHE_PREFIX + "email:" + user.getEmail(), user, USER_CACHE_EXPIRE);
        }
    }
...
//判断密码是否为空
    ...
    public void deleteBatch(List<Integer> ids) {
        List<User> users = ids == null || ids.isEmpty() ? List.of() : userMapper.selectBatchIds(ids);
    ...
    public Page<User> getUsersByPage(String username, String sex, String nickname, String roleCode, Integer currentPage, Integer size) {
        ...
        // 添加查询条件
        //StringUtils.isNotBlank(参数),判断参数不为空、不为 null、不全是空格，满足才会拼接条件
